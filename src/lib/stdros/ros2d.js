    ...
    !(function (undefined$1) {
      ...
      function resolveOptions(options, schema, reducers, allowUnknown) {
        ...
        function reject(reason) {
          throw Error(
            'Invalid "' + option + '" option value' + (reason ? '. Reason: ' + reason : ''),
          )
        }
      ...
      EventEmitter.prototype.hasListeners = function (type) {
        ...
        return !!(
          (_all && _all.length) ||
          (_events && (type === undefined$1 ? ownKeys(_events).length : _events[type]))
        )
      }
...
Viewer.prototype.scaleToDimensions = function scaleToDimensions(width, height) {
  ...
  this.scene.x =
    typeof this.scene.x_prev_shift !== 'undefined' ? this.scene.x_prev_shift : this.scene.x
  this.scene.y =
    typeof this.scene.y_prev_shift !== 'undefined' ? this.scene.y_prev_shift : this.scene.y
...
ZoomView.prototype.zoom = function zoom(zoom$1) {
  ...
  this.stage.x =
    this.startShift.x -
    (this.center.x - this.startShift.x) * (this.stage.scaleX / this.startScale.x - 1)
  this.stage.y =
    this.startShift.y -
    (this.center.y - this.startShift.y) * (this.stage.scaleY / this.startScale.y - 1)
}
